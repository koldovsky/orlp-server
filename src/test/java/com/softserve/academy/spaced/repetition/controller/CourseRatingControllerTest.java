...
public class CourseRatingControllerTest {
    ...
    public void getCourseRatingById() throws Exception {
        when(courseRatingService.getCourseRatingById(eq(5L))).thenReturn(createCourseRating());
        mockMvc.perform(get("/api/rate/course/{id}", 5L)
                .accept(MediaType.APPLICATION_JSON)
                .contentType(MediaType.APPLICATION_JSON))
                .andExpect(status().isOk())
                .andExpect(content().json("{\"rating\":3,\"accountEmail\":\"email@email\",\"courseId\":5,\"links\":[{\"rel\":\"self\",\"href\":\"http://localhost/api/rate/course/77\"}]}"));
    }
    ...
    public void testAddCourseRating() throws Exception {
        ...
        verify(courseRatingService, times(1)).addCourseRating(any(CourseRating.class), eq(5L));
    }

    @Test
    public void testNotAuthorizedAddCourseRating() throws Exception {
        doThrow(NotAuthorisedUserException.class).when(courseRatingService).addCourseRating(any(CourseRating.class), eq(5L));
