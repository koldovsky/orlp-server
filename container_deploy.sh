...
if [ ! "$(docker ps -aq -f name=$container_name)" ]; then
    ...
    docker run --name $container_name --net=orlp --add-host orlp-mysql:192.168.0.50 -p 8080:8080 --expose=1443 -e 'VIRTUAL_HOST=infolve.com:8080' -e 'VIRTUAL_PROTO=https' -v /home/wercker/orlp-server:/usr/src/myapp -w /usr/src/myapp java:8-jre java -jar Spaced.Repetition.jar;
    else
