...
if [ ! "$(docker ps -aq -f name=$container_name)" ]; then
    	...
    	docker run --name $container_name -d --restart=always --net=orlp --add-host orlp-mysql:192.168.0.50 --expose=80 -e 'VIRTUAL_HOST=api.infolve.com' -e 'LETSENCRYPT_EMAIL=$LETSENCRYPT_EMAIL' -e 'LETSENCRYPT_HOST=api.infolve.com' -v /home/wercker/orlp-server:/usr/src/myapp -w /usr/src/myapp java:8-jre java -Dserver.port=80 -jar Spaced.Repetition.jar;
    else
