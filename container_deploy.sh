...
if [ ! "$(docker ps -aq -f name=$container_name)" ]; then
    	...
    	docker run --name $container_name -d --restart=always --net=orlp --add-host orlp-mysql:192.168.0.50 --expose=1443 -e 'VIRTUAL_HOST=api.infolve.com' -e 'VIRTUAL_PORT=1443' -e 'LETSENCRYPT_EMAIL=holovko.oleksandr@gmail.com' -e 'LETSENCRYPT_HOST=api.infolve.com' -v /home/wercker/orlp-server:/usr/src/myapp -w /usr/src/myapp java:8-jre java -jar Spaced.Repetition.jar;
    else
